...
use super::compiler::ResolvedVariable;
    ...
    fn expr_bp(&mut self, min_bp: u8) -> Result<(), SyntaxError> {
        ...
        match self.peek().token_type.clone() {
            TokenType::Number => {
                ...
                self.compiler.emit_constant(value, span);
            }
            TokenType::String => {
                let span = self.next()?.span;
                let lexeme = self.lexeme(&span);
            ...
            TokenType::True => {
                let span = self.next()?.span;
                self.compiler.emit_byte(OpCode::True, span);
            }
            TokenType::False => {
                let span = self.next()?.span;
                self.compiler.emit_byte(OpCode::False, span);
            }
            TokenType::Nil => {
                let span = self.next()?.span;
                self.compiler.emit_byte(OpCode::Nil, span);
            }
            TokenType::LeftParen => {
                let _ = self.next();
                self.expr_bp(0)?;
            ...
            TokenType::Identifier => {
                let identifier = self.identifier("Exoect identifier.")?;
                let resolved = self.compiler.resolve_variable(&identifier)?;

                match (resolved, &self.peek().token_type) {
                    (ResolvedVariable::Local(index), TokenType::Equal) if min_bp == 0 => {
                    ...
                    (ResolvedVariable::Global, TokenType::Equal) if min_bp == 0 => {
                        let span = self.next().expect("should be equal token").span;
                        self.expression()?;
                        self.compiler.emit_set_global(Identifier {
            ...
            TokenType::Minus | TokenType::Bang => {
                ...
                self.expr_bp(r_bp)?;
                self.compiler.emit_byte(opcode, token.span);
            }
            _ => {
                let span = self.peek().span.clone();
                return Err(SyntaxError {
        ...
        loop {
            ...
            if let Some((l_bp, r_bp)) = infix_binding_power(&op.token_type) {
                ...
                if matches!(&op.token_type, TokenType::And) {
                    let jump = self
                        .compiler
                        .emit_jump(OpCode::JumpIfFalse, op.span.clone());
                    self.compiler.emit_byte(OpCode::Pop, op.span.clone());
                    self.expr_bp(r_bp)?;
                    ...
                    let else_jump = self
                        .compiler
                        .emit_jump(OpCode::JumpIfFalse, op.span.clone());
                    let end_jump = self.compiler.emit_jump(OpCode::Jump, op.span.clone());
                    ...
                    self.compiler.emit_call(arg_count, span);
                } else {
                    ...
                    for code in opcodes.iter().cloned() {
                        self.compiler.emit_byte(code, op.span.clone());
                    }
                }
            } else {
                break;
